...
2. **Create an Environment:** Open Anaconda Navigator (or use the `conda` command), create an environment named `syno-ai`, and select Python 3.12.
3. **Install Docker Desktop:** Download and install Docker Desktop from [here](https://www.docker.com/products/docker-desktop/).
4. **Download Syno AI:** Download the latest release zip from the [GitHub releases page](https://github.com/synotechai/syno-ai/releases) and extract it.
5. **Configure API Keys:** Duplicate `example.env`, rename it to `.env`, and add your API keys.
6. **Install Requirements:** Activate your `syno-ai` environment, open a terminal in the Syno AI directory, and run: `pip install -r requirements.txt`
7. **Run Syno AI (Web UI):** Execute `python run_ui.py` in your terminal.
...
3. **Download Syno AI:** Download the latest release zip from the [GitHub releases page](https://github.com/synotechai/syno-ai/releases) and extract it.
4. **Open Terminal:** Navigate to the Syno AI directory.
...
9. **Run Syno AI (Web UI):** Run: `python run_ui.py`
10. **Access the Web UI:** Open the displayed URL.

> ⚠️ **Important macOS Docker Configuration:** In Docker Desktop's preferences (Docker menu) go to Settings, navigate to "Advanced" and check "Allow the default Docker socket to be used (requires password)."  This allows Syno AI to communicate with the Docker daemon.
...
4. **Download Syno AI:** Download the latest release zip from the [GitHub releases page](https://github.com/synotechai/syno-ai/releases) and extract it.
5. **Open Terminal and Navigate:** Open your terminal and navigate to the extracted Syno AI folder using the `cd` command.
...
9. **Run Syno AI (Web UI):** `python run_ui.py`
10. **Access the Web UI:** Open the URL displayed in your terminal.

</details>

# In-Depth Guide for Windows and macOS
- Syno AI is not an app. It's a framework. It's made to be customized, edited, enhanced. Therefore you need to install the necessary components to run it.
- The following step by step instructions can be followed along with a video for this tutorial on how to make Syno AI work on Windows and macOS that you find in the [top section](#installation-guide-for-windows-macos-and-linux) of the page.

## Reminders:
1. There's no need to install Python, Conda will manage that for you.
2. You don't necessarily need API keys: Syno AI can run with local models. For this tutorial though, we will leave it to the default OpenAI API. A simple video explanation for running Free and Local models is available [in this video](https://www.youtube.com/watch?v=sLMhLRHDIPY&t=80s).
3. Visual Studio Code or any other code editor is not mandatory, but it makes it easier to navigate and edit files.
4. Docker is not mandatory, but running Syno AI without Docker is actually more complicated and can be very dangerous, so we will be covering the easier and recommended way - using Docker Desktop application.
...
- Simply put, Docker is a way of running virtual computers on your machine. These are lightweight, disposable and isolated from your operating system, so it is a way to sandbox Syno AI.
- Syno AI only connects to the Docker container when it needs to execute code and commands. The frameworks itself runs on your machine.
- Running Syno AI without Docker is possible, but more complicated and dangerous, I will not be covering that in this tutorial.
...
> ⚠️ **Important macOS-only Docker Configuration:** In Docker Desktop's preferences (Docker menu) go to Settings, navigate to "Advanced" and check "Allow the default Docker socket to be used (requires password)."  This allows Syno AI to communicate with the Docker daemon.

![docker socket macOS](res/setup/macsocket.png)

## 3. Download Syno AI
- You can clone the Syno AI repository (https://github.com/synotechai/syno-ai) from GitHub if you know how to use Git. In this tutorial I will just show how to download the files.

1. Go to the Syno AI releases [here](https://github.com/synotechai/syno-ai/releases).
2. The latest release is on the top of the list, click the "Source Code (zip)" button under "Assets" to download it.

<img src="res/setup/image-14.png" alt="syno ai download" width="500"/>
<br><br>

3. Extract the downloaded archive where you want to have it. I will extract them to "syno-ai" folder on my Desktop - "C:\Users\synotechai\Desktop\syno-ai" on Windows and "/Users/synotechai/Desktop/syno-ai" on macOS.
...
2. In the terminal, navigate to your Syno AI folder using **"cd"** command. Replace the path with your actual Syno AI folder path.
~~~
cd C:\Users\synotechai\Desktop\syno-ai
~~~
You should see your folder has changed on the next terminal line.

<img src="res/setup/image-15.png" alt="syno ai cd" height="100"/>
<img src="res/setup/image-16.png" alt="syno ai cd" height="100"/>
<br><br>

3. Create Conda environment using command **"conda create"**. After **"-n"** is your environment name, you can choose your own, i will use **"a0"** - short for Syno AI. After **"python"** is the Python version that Conda will install for you into this environment, right now, 3.12 works fine. **-y** skips confirmations.
~~~
conda create -n a0 python=3.12 -y
~~~
...
## 5. Configure Syno AI
- Now we can configure Syno AI - select models, settings, API Keys etc.
- I will leave the default configuration using OpenAI API and I will just provide my OpenAI API key in the **.env** file.

1. Find the **"example.env"** file in your Syno AI folder and edit the contents. I will put my OpenAI API key after **"API_KEY_OPENAI="**. If you have API keys for other service providers, add them as needed.
...
## 6. Run Syno AI
- Setup done. It's time to test Syno AI. Let's double check:
...
2. Make sure your terminal is looking into the Syno AI folder. If not, navigate to it with **"cd"** command again with your path.
...
5. Open the URL shown in terminal in your web browser. You should see the Syno AI interface. The first time it needs to execute code, Docker image will be downloaded and deployed, this might take some time. Be patient when seeing "Initializing docker container syno-ai-exe for safe code execution...".
...
## Choosing Your LLMs
The `initialize.py` file is the control center for selecting the Large Language Models (LLMs) that power Syno AI.  You can choose different LLMs for different roles:
...
To update Syno AI to the latest version, follow these steps:
- **Using Git/GitHub:** Pull the latest version of the Syno AI repository with Git/GitHub. The custom knowledge, solutions, memory, and other data will get ignored by Git, so you don't need to worry about losing any of your custom data. The same goes for your .env file with all of your API keys.
...
- **Manual Update:** Alternatively, you can manually download the latest version of Syno AI from the [releases page](https://github.com/synotechai/syno-ai/releases). The only folders that you must save to retain custom data are the following: `/memory`, `/knowledge`. If you have created custom `instruments` or `prompts` you must save their dirs as well.
...
# How to create executable files for Syno AI in Windows, macOS and Linux
Two scripts allows you to easily create executable files for Windows, macOS and Linux, including all the necessary dependencies to run Syno AI without the need for Python, Conda, or additional steps:
1. **Navigate to Bundle folder:** Navigate to /syno-ai/bundle.
2. **Run the script:** Run the bundling of Syno AI by running `macos_bundle.sh` (macOS and Linux) or `windows_bundle.bat` (Windows) in your terminal window. The process usually takes about 2 to 5 minutes depending on the platform.
3. **Edit your Files:** Now you can configure API keys and edit `initialize.py` for [Choosing Your LLMs](#choosing-your-llms).
4. **Run Syno AI:** Run Syno AI by running the .exe file just created (Windows), or by running `./syno-ai` in your terminal window.

- ⚠️ **IMPORTANT:** The 2 scripts will create executable files valid only for the machine where they are run. 

If you want to run Syno AI on another machine, you will need to run the scripts on that machine as well to have executable files.
      
## Conclusion
After following the instructions for your specific operating system, you should have Syno AI successfully installed and running. You can now start exploring the framework's capabilities and experimenting with creating your own intelligent agents.  If you encounter any issues during the installation process, please consult the Troubleshooting section of this documentation or refer to the Syno AI community for assistance.