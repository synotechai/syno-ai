...
Syno AI is built on a flexible and modular architecture designed for extensibility and customization. This document outlines the key components and their interactions. This simplified diagram illustrates the hierarchical relationship between agents and their interaction with tools, prompts, memory, knowledge base and instruments. 
...
6. **Instruments:** Instruments provide a way to add custom functionalities to Syno AI. They can modify agent behavior introducing new procedures, call for tools, or scripts to integrate with other systems. Instruments scripts are run inside the Docker container.

7. **Extensions:** Extensions are custom components that can be added to Syno AI to enhance its capabilities. Extensions can include new message loops, custom memory management rules, or even more.

8. **Docker Container:** Syno AI execute code commands in a Docker container, which provides isolation and security. This ensures that agents can safely and securely interact with tools and external systems.
...
## Knowledge
- **Knowledge Base:** Wiki, books, code documentation, and other documents can be added to the knowledge base. Syno AI uses this information to answer questions and make decisions. The `/docs` folder, containing Syno AI's documentation, is automatically added to the knowledge base.
...
| `example.env` | Contains environment variables for configuring API keys and other settings of Syno AI. |
...
## Adding Instruments
Instruments allow you to add predefined actions or workflows to Syno AI without adding to the token count of the system prompt.
