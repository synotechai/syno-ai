...
3. **Full Binaries**
- Using Git/GitHub: Pull the latest version of the Syno AI repository with. The custom knowledge, solutions, memory, and other data will get ignored, so you don't need to worry about losing any of your custom data. The same goes for your .env file with all of your API keys and settings.json.
...
- Syno AI is a framework. It's made to be customized, edited, enhanced. Therefore you need to install the necessary components to run it when downloading its full binaries. This guide will help you to do so.
- The following step by step instructions can be followed along with a video for this tutorial on how to make Syno AI work with its full development environment.
...
2. You don't necessarily need API keys: Syno AI can run with local models. For this tutorial though, we will leave it to the default OpenAI API. A guide for downloading Ollama along with local models is available [here](#installing-and-using-ollama-local-models).
...
6. Running Syno AI without Docker makes the process more complicated and it's thought for developers and contributors.
...
<img src="res/setup/image-14-u.png" alt="syno ai download" width="500"/>
<br><br>
...
- Syno AI needs a Docker image to be pulled from the Docker Hub to be run, even when using the full binaries.
You can refer to the [installation instructions above](#windows-macos-and-linux-setup-guide) to run the Docker container and then resume from the next step. There are two differences:
  ...
  - You need to map the `/a0` volume to the location of your local Syno AI folder.
- Run the Docker container following the instructions.

## 5. Run the local Syno AI instance
Run the Syno AI with Web UI:
~~~
python run_ui.py
~~~
...
- Open the URL shown in terminal in your web browser. You should see the Syno AI interface.

## 6. Configure Syno AI
Now we can configure Syno AI - select models, settings, API Keys etc. Refer to the [Usage](usage.md#agent-configuration) guide for a full guide on how to configure Syno AI.

## 7. Configure Syno AI RFC
Syno AI needs to be configured further to redirect some functions to the Docker container. This is crucial for development as A0 needs to run in a standardized environment to support all features.
...
9. Now you have the full development environment to work on Syno AI.
...
### Conclusion
After following the instructions for your specific operating system, you should have Syno AI successfully installed and running. You can now start exploring the framework's capabilities and experimenting with creating your own intelligent agents. 
